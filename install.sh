#!/usr/bin/env bash
# =============================================================================
# sip4ai — Self-Hosted Installer
# Installs the full sip4ai platform on a fresh Ubuntu 22.04/24.04 server.
#
# Usage:
...
REPO_URL="${SIP4AI_REPO_URL:-https://github.com/YOUR_ORG/sip4ai.git}"

APP_USER="sip4ai"
INSTALL_DIR="/opt/sip4ai"
...
# ── Step 4: sip4ai binary ─────────────────────────────────────────────────────
step "Setting up sip4ai binary"

SIP4AI_BIN="${INSTALL_DIR}/.bin/sip4ai"
BIND_SO="${INSTALL_DIR}/.bin/bind_override.so"

[[ -f "${SIP4AI_BIN}" ]] || die "Binary not found at ${SIP4AI_BIN}. Make sure the .bin/ directory is committed to the repository."
...
success "sip4ai binary ready at ${SIP4AI_BIN}"
...
# sip4ai configuration — generated by install.sh on $(date -u +"%Y-%m-%dT%H:%M:%SZ")
...
sudo -u "${APP_USER}" bash -c "
    set -e
    cd '${INSTALL_DIR}'
    BASE_PATH=/ NODE_ENV=production pnpm --filter @workspace/sip4ai-manager run build 2>&1
" || die "Frontend build failed."
success "Frontend built → artifacts/sip4ai-manager/dist/public"
...
cat > /etc/systemd/system/sip4ai.service <<EOF
[Unit]
Description=sip4ai API Server
Documentation=https://github.com/YOUR_ORG/sip4ai
...
SyslogIdentifier=sip4ai

# Allow the sip4ai child processes to bind privileged SIP ports
...
systemctl enable sip4ai --quiet
systemctl restart sip4ai || die "Failed to start sip4ai service. Check: journalctl -u sip4ai -n 50"
success "sip4ai systemd service enabled and started"

# ── Step 12: nginx reverse proxy ──────────────────────────────────────────────
step "Configuring nginx"

STATIC_ROOT="${INSTALL_DIR}/artifacts/sip4ai-manager/dist/public"

cat > /etc/nginx/sites-available/sip4ai <<EOF
# sip4ai — generated by install.sh
server {
...
ln -sf /etc/nginx/sites-available/sip4ai /etc/nginx/sites-enabled/sip4ai
rm -f /etc/nginx/sites-enabled/default

nginx -t || die "nginx configuration test failed. Check /etc/nginx/sites-available/sip4ai"
...
if [[ "${API_HTTP_CODE}" == "200" ]]; then
    ...
    warn "API server returned HTTP ${API_HTTP_CODE}. Check: journalctl -u sip4ai -n 50"
    ALL_OK=false
fi
...
if [[ "${ALL_OK}" == false ]]; then
    ...
    echo "  journalctl -u sip4ai -n 80 --no-pager   # API server logs"
    echo "  journalctl -u nginx  -n 30 --no-pager   # nginx logs"
    echo "  systemctl status sip4ai                 # service status"
fi

echo ""
echo "Useful commands:"
echo "  journalctl -u sip4ai -f      # stream API server logs"
echo "  systemctl restart sip4ai     # restart the API server"
echo "  systemctl status sip4ai      # service health"
