...
# ── Step 10b: sudoers — allow app user to manage nginx + certbot ─────────────
step "Configuring sudoers for nginx/certbot management"

SUDOERS_FILE="/etc/sudoers.d/aio-voice-connect"
cat > "${SUDOERS_FILE}" <<EOF
# Allow ${APP_USER} to manage nginx config and certbot without a password.
# Generated by install.sh — do not edit manually.
${APP_USER} ALL=(ALL) NOPASSWD: /usr/bin/tee /etc/nginx/sites-available/aio-voice-connect.conf
${APP_USER} ALL=(ALL) NOPASSWD: /bin/ln -s /etc/nginx/sites-available/aio-voice-connect.conf /etc/nginx/sites-enabled/aio-voice-connect.conf
${APP_USER} ALL=(ALL) NOPASSWD: /bin/rm -f /etc/nginx/sites-enabled/aio-voice-connect.conf
${APP_USER} ALL=(ALL) NOPASSWD: /usr/sbin/nginx -t
${APP_USER} ALL=(ALL) NOPASSWD: /bin/systemctl reload nginx
${APP_USER} ALL=(ALL) NOPASSWD: /usr/bin/certbot --nginx *
EOF
chmod 440 "${SUDOERS_FILE}"
visudo -cf "${SUDOERS_FILE}" || die "Generated sudoers file is invalid — check ${SUDOERS_FILE}"
...
nginx -t || die "nginx configuration test failed. Check /etc/nginx/sites-available/aio-voice-connect"
